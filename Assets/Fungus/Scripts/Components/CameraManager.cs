...
namespace Fungus
{
    ...
    public class CameraManager : MonoBehaviour
    {
            ...
            LeanTweenType posTweenType = LeanTweenType.easeInOutQuad, LeanTweenType rotTweenType = LeanTweenType.easeInOutQuad)
        {
            ...
            {
                // 如果需要，这里可以尝试直接执行 fadeAction 以避免卡死，但在这种异常状态下直接 return 更安全
                return;
            }
            ...
            Fade(1f, outDuration, delegate {
                ...
                {
                    // 如果对象丢了，就不执行移动，直接尝试淡入回来，防止屏幕一直黑着
                    // 或者直接 return 停止操作
                    //Debug.LogWarning("FadeToView execution stopped because View or Camera was destroyed during the fade out.");
                    
                    // 尝试恢复画面显示（淡入），否则玩家会看到全黑屏幕
        ...
        protected void StopFadeTween()
        {
            if (fadeTween != null)
            {
                LeanTween.cancel(fadeTween.id, false);
                fadeTween = null;
            }
        }
        ...
        protected void StopPosTweens()
        {
            if (sizeTween != null)
            {
                LeanTween.cancel(sizeTween.id, false);
                sizeTween = null;
            }
            if (camPosTween != null)
            {
                LeanTween.cancel(camPosTween.id, false);
                camPosTween = null;
            }
            if (camRotTween != null)
            {
                LeanTween.cancel(camRotTween.id, false);
                camRotTween = null;
            }
        }
