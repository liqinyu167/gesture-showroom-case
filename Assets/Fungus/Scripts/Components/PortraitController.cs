...
namespace Fungus
{
    ...
    public class PortraitController : MonoBehaviour
    {
        ...
        {
            if (options.character != null && options.character.State != null && options.character.State.allPortraits != null)
            {//清除多余立绘
                foreach (var img in options.character.State.allPortraits)
                {
                    if (img == null) continue;

                    // 判定条件：
                    // 1. 它是开着的 (Active)
                    // 2. 它不是我们要显示的新立绘 (img.sprite != options.portrait)
                    // 3. 它不是当前主要的立绘 (img != options.character.State.portraitImage) -> 也就是那个正准备淡出的
                    bool isTarget = (img.sprite == options.portrait);
                    bool isCurrent = (img == options.character.State.portraitImage);

                    if (img.gameObject.activeSelf && !isTarget && !isCurrent)
                    {
        ...
        protected virtual void HidePortrait(RectTransform rectTransform, float duration)
        {
            ...
            // 只有当物体还存在时，才去执行 SetActive(false)
            // 这防止了如果 destroy 后回调依然被触发导致的 MissingReferenceException
