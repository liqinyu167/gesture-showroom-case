...
namespace Mediapipe.Unity.Sample.MediaPipeVideo
{
    public class MediaPipeVideoGraph : GraphRunner
    {
        ...
        public async Task<(ImageFrame, List<NormalizedLandmarkList>, List<ClassificationList>)> WaitNextAsync()
        {
            var videoTask = _outputVideoStream.WaitNextAsync();
            var landmarksTask = _handLandmarksStream.WaitNextAsync();
            var handednessTask = _handednessStream.WaitNextAsync();

            await Task.WhenAll(videoTask, landmarksTask, handednessTask);

            var videoResult = videoTask.Result;
            AssertResult(videoResult);

            var landmarksResult = landmarksTask.Result;
            AssertResult(landmarksResult);

            var handednessResult = handednessTask.Result;
            AssertResult(handednessResult);

            _ = TryGetValue(videoResult.packet, out var outputVideo, (packet) =>
            {
                return packet.Get();
            });

            _ = TryGetValue(landmarksResult.packet, out var handLandmarks, (packet) =>
            {
                return packet.Get(NormalizedLandmarkList.Parser);
            });

            _ = TryGetValue(handednessResult.packet, out var handedness, (packet) =>
            {
                return packet.Get(ClassificationList.Parser);
            });

            return (outputVideo, handLandmarks ?? new List<NormalizedLandmarkList>(), handedness ?? new List<ClassificationList>());
        }
