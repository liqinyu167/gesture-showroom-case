...
namespace Mediapipe.Unity
{
  public class WebCamSource : ImageSource
  {
    ...
    private const int DeviceEnumerationRetryCount = 60;
...
#pragma warning restore IDE0025
    ...
    private IEnumerator Initialize()
    {
      ...
      for (var i = 0; i < DeviceEnumerationRetryCount; i++)
      {
        ...
        if (i == 0)
        {
          Debug.LogWarning($"{_TAG}: No WebCam devices found yet. Retrying device enumeration...");
        }
      ...
      if (availableSources != null && availableSources.Length > 0)
      {
        ...
        Debug.Log($"{_TAG}: Selected camera \"{webCamDevice.Value.name}\" from {availableSources.Length} device(s).");
      }
      else
      {
        Debug.LogError($"{_TAG}: No WebCam devices found. Check Windows camera privacy settings and make sure the camera is not being used by another app.");
      }
    }
